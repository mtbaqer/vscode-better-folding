...
export function activate(context: ExtensionContext) {
  const isNativeBracketPairColorizationEnabled = !!workspace
    .getConfiguration()
    .get<boolean>("editor.bracketPairColorization.enabled");
  ...
  if (showNotice) {
    ...
    window.showInformationMessage("Bracket Pair Colorizer is no longer being maintained.", ...items).then((e) => {
      ...
      if (e?.title == "Migrate to native colorization") {
        if (!isNativeBracketPairColorizationEnabled) {
          workspace
            .getConfiguration()
            .update("editor.bracketPairColorization.enabled", true, ConfigurationTarget.Global);
        ...
        commands.executeCommand("workbench.extensions.uninstallExtension", "CoenraadS.bracket-pair-colorizer-2");
      }
